...
int main()
{
    ...
    ring_buffer_t<vector<vector<double>>> buffer(2, { odin, odin }); //создание буфера из 2 векторов: серы и плотности, размер 2 слоя
    ...
    for (size_t i = 0; i < myPipe.N; i++) {
        ...
        buffer.advance(1);//сместим индекс текущего слоя в буфере на единицу
    }
